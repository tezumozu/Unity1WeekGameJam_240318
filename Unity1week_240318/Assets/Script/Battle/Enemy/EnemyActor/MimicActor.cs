...
public class MimicActor : BattleActor{

    private List<S_EnemySkillData> enemySkillList;
    private Dictionary<E_ActionType,bool> SPActionDic;


    public MimicActor(I_ActionCreatable actionFactory,I_BuffCreatable buffFactory,I_StatusEffectCreatable statusEffectFactory):base(actionFactory,buffFactory,statusEffectFactory){
        ...
        //ステータス読み込み
        //パスを生成
        var fileName = "BattleScene/Enemy/Mimic";
        //読み込む
        ...
        SPActionDic[E_ActionType.SilenceCurse] = true;
    }
        ...
        if(currentAction.IsNextAction){
            ...
            switch ((float)currentStatus.HP / (float)maxStatus.HP){
                case float x when x < 0.5f :
                    if(SPActionDic[E_ActionType.SilenceCurse]){
                        skillType = E_ActionType.SilenceCurse;
                        SPActionDic[E_ActionType.SilenceCurse] = false;
                    }
                    break;
            }

            //HP50%以下の時、SleepBiteはSlenceBiteになる
            if(skillType == E_ActionType.SleepBite && (float)currentStatus.HP / (float)maxStatus.HP < 0.5f){
                skillType = E_ActionType.SilenceBite;
            }
